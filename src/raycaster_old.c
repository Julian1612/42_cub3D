...
/*   Updated: 2023/03/20 21:59:08 by lorbke           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
// double	extend_ray(mlx_image_t *image, t_map *map, t_player *player, double ray_dir)
// {
// 	int		x;
// 	int		y;
// 	double	dx;
...
// 	double	x_intercept;
// 	double	y_intercept;
// 	bool	(*func_y)(double, double);
// 	bool	(*func_x)(double, double);

// 	x = player->x / MM_BLOCK_SIZE;
// 	y = player->y / MM_BLOCK_SIZE;
// 	dx = fmod(player->x, UNIT);
// 	dy = fmod(player->y, UNIT);
// 	if (cos(ray_dir) > 0)
// 		y_tile = UNIT;
// 	else
// 		y_tile = UNIT * -1;
...
// 	else
// 		func_x = greater_than;
// 	y_step = UNIT / tan(ray_dir);
// 	if (y_step >= 0)
// 		func_y = greater_than;
// 	else
// 		func_y = less_than;
// 	x_intercept = x + dx - dy / tan(ray_dir);
// 	y_intercept = y + dy + dx * tan(ray_dir);
// 	while (1)
// 	{
// 		while (func_y(y_intercept, y))
// 		{
// 			if (map->map[x][(int)y_intercept] == '1')
// 				return (sqrt(get_abs(x_intercept * x_intercept) + get_abs(y_intercept * y_intercept)));
// 			x += x_tile;
// 			y_intercept += y_step;
// 		}
// 		while (func_x(x_intercept, x))
// 		{
// 			if (map->map[(int)x_intercept][y] == '1')
// 				return (sqrt(get_abs(x_intercept * x_intercept) + get_abs(y_intercept * y_intercept)));
// 			y += y_tile;
// 			x_intercept += x_step;
// 		}
...
double	cast_ray(t_game *game, double ray_dir)
{
	...
	ray_len = extend_ray(game->img_a, &game->map, &game->player, ray_dir);
	printf("ray_dir: %f\n", ray_dir);
	return (ray_len);
